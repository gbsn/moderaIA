...
# stream_out: Saída para caixas ajustada para buffer de acumulação (1024 é mais estável aqui)
stream_out = p.open(format=pyaudio.paInt16, channels=1, rate=24000, output=True, frames_per_buffer=1024)
    ...
    while True:
        ...
        if rec.AcceptWaveform(data):
            ...
            if user_input.strip():
                ...
                # --- GERAÇÃO DE VOZ E ESTRATÉGIA ANTI-CHIADO ---
                ...
                # ACUMULAÇÃO: Coletamos todos os pedaços antes de enviar para a placa de som
                buffer_voz = []
                for chunk in audio_stream:
                    if chunk:
                        buffer_voz.append(chunk)
                
                # Une todos os pedaços em um único bloco sólido de áudio
                audio_final = b"".join(buffer_voz)

                # FLUSH DE HARDWARE: Garante que o canal de saída esteja limpo
                stream_out.stop_stream()
                stream_out.start_stream()

                # TRANSMISSÃO: Toca o bloco inteiro de uma vez, eliminando engasgos da rede
                stream_out.write(audio_final)
